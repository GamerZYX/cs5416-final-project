...
class test_doc_rerank: 
    ...
    def __init__(self, tracker): 
        ...
        self.reranker_model = AutoModelForSequenceClassification.from_pretrained(self.reranker_model_name).eval()
        self.tracker.set_event(f"{self.NAME}_LOAD_END")
        print(f"Loaded rerankers.")
    ...
    def run_test(self, batch_size, n_iters=10):
        ...
        for _ in range(batch_size): 
            query = random.choice(test_doc_rerank.TEST_QUERIES)
...
if __name__=="__main__": 
    ...
    memory_tracker = MemoryTracker(interval=0.1, output_file=f"./test_results/{test_doc_rerank.NAME}_mem.csv")
    memory_tracker.start()
    
    # 2. Initialize the test runner, which triggers model loading events
    try:
        tester = test_doc_rerank(memory_tracker)
